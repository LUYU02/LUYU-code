...
<template>
  <div v-show="props.isShow" class="checkout">
    ...
    <div class="details">
      <header>餐品详情</header>
      <div class="list">
        <div class="item" v-for="item in meals.shoppingCart">
          <div class="img-wrap">
            <img :src="item.img" :alt="item.title" style="height: 160rem" />
          </div>
          <div class="info">
            <h2 class="title">{{ item.title }}</h2>
            <div class="count">
              <Counter :meal="item" @close="$emit('close')"></Counter>
      ...
      <footer>
        合计￥<span>{{ meals.sumBuyHamburgPrice }}</span>
      </footer>
    </div>

    <div class="bar">
      <div class="total-amount">
        <p v-show="meals.sumBuyHamburg <= 0" class="no-goods">未选购商品</p>
        <p
          @click="showDetails = true"
          v-show="meals.sumBuyHamburg > 0"
          class="has-goods">
